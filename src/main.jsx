...
function App() { const [session, setSession] = useState(null); useEffect(() => { if (!supabase) return; supabase.auth.getSession().then(({ data }) => setSession(data.session)); return supabase.auth.onAuthStateChange((_event, next) => setSession(next)).data.subscription.unsubscribe }, []); if (!isSupabaseConfigured) return <main className="login"><section className="card"><h1>Configuração necessária</h1><p>Defina uma URL HTTP(S) válida em <code>VITE_SUPABASE_URL</code> e uma anon key em <code>VITE_SUPABASE_ANON_KEY</code> antes de publicar este site.</p></section></main>; return session ? <Dashboard session={session} /> : <Login /> }
